...
from src.memzero.pipeline_add import MemoryADD
from src.memzero.pipeline_search import MemorySearch
...
class Experiment:
    def __init__(self, technique_type, chunk_size):
        self.technique_type = technique_type  # 记忆技术类型
        self.chunk_size = chunk_size  # 数据块大小

    def run(self):
        print(f"使用 {self.technique_type} 记忆技术运行实验, 数据块大小: {self.chunk_size}")
...
def main():
    ...
    print(f"使用: {args.technique_type} 记忆技术运行实验, 数据块大小: {args.chunk_size}")
