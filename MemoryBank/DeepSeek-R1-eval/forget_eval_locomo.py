...
from memory_bank.memory_retrieval.forget_memory_locomo import LocalMemoryRetrieval, load_memory_file
import json, time
import openai
import pathlib
PROJECT = pathlib.Path(__file__).resolve().parents[1]   # 向上跳一级到 eval-data
...
for qa_idx, qa in enumerate(qa_pairs):
    ...
    related_memories, memory_dates = memory_system.search_memory(
        query=question, vector_store=vector_store, max_dates=3, max_chars_per_date=300)
    system_prompt = (
    ...
    print(f"发送给AI的messages：{message}")
